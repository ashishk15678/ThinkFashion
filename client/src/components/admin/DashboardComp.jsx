...
const DashboardComp = () => {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        const res = await fetch(`${root}/users/getUsers?perPage=5`, { headers });
        const data = await res.json();
        if (res.ok) {
          setUsers(data.users);
          setTotalUsers(data.totalUsers);
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${root}/product/all?page=1&perPage=5`, { headers });
        const data = await res.json();
        if (res.ok) {
          setProducts(data.products);
          setTotalProducts(data.totalProducts);
    ...
    const fetchCountProducts = async () => {
      try {
        const res = await fetch(`${root}/product/count`, { headers });
        const data = await res.json();
        if (res.ok) setCountProduct(data.counts);
      } catch (err) {
        console.error(err);
      }
    };
    ...
    const fetchComments = async () => {
      try {
        const res = await fetch(`${root}/comment/getAllComment`, { headers });
        const data = await res.json();
        if (res.ok) setCountComment(data.totalComments);
      } catch (err) {
        console.error(err);
      }
    };
  ...
  return (
    <div className="px-3 dark:bg-gray-900 py-6 lg:py-8">
      <div className="flex-wrap flex gap-4 justify-center">
        {/* Total Users */}
        <div className="flex flex-col p-3 dark:bg-gray-800 gap-4 md:w-72 w-full rounded-md shadow-md">
          <div className="flex justify-between">
      ...
      {/* Users Table */}
      <div className="flex flex-wrap gap-4 lg:gap-10 py-6 lg:px-8 justify-center">
        <div className="flex flex-col w-full md:w-auto shadow-md p-2 rounded-md dark:bg-gray-800">
          <div className="flex justify-between p-3 text-sm font-semibold">
            <h1 className="text-center p-2">Recent Users</h1>
            <Button outline gradientDuoTone="purpleToPink">
        ...
        {/* Product Percentage */}
        <div className="flex flex-col w-full md:w-auto shadow-md p-2 rounded-md dark:bg-gray-800 gap-10">
          <h3 className="text-center text-lg py-3">Product Percentage</h3>
          <div className="flex flex-col justify-center items-center gap-8">
            {["men", "women", "kids"].map((key) => (
              <div className="flex gap-2 items-center" key={key}>
                <h4 className="text-sm capitalize">{key}</h4>
                <progress
                  className="progress progress-secondary w-44 mt-2 dark:bg-white"
                  value={countProduct[key] || 0}
                  max="100"
                />
                <span className="text-sm">{`${countProduct[key] || 0}%`}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            {products.map((product) => (
              <Table.Body key={product._id} className="divide-y">
                <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
                  ...
                  <Table.Cell className="w-96">{product.name}</Table.Cell>
                  <Table.Cell className="w-5">{product.category.name}</Table.Cell>
                </Table.Row>
              </Table.Body>
            ))}
          </Table>
