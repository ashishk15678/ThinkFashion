...
const addProduct = asyncHandler(async (req, res) => {
  ...
  if ([description, name, price, stock].some(field => !field || field.trim() === "")) {
  ...
  if (!parsedCategory?.name || !parsedCategory?.type) {
  ...
  const uploadResult = await uploadToCloudinary(fileUri, file.originalname);
  if (!uploadResult.success) throw new ApiError(500, "Error uploading image");

  const newProduct = await Product.create({
    name,
    description,
    productImage: uploadResult.result?.secure_url,
    price,
    stock,
  ...
  const createdProduct = await Product.findById(newProduct._id);
  res.status(201).json(new ApiResponse(200, createdProduct, "Product added successfully"));
});
...
const getProductsByCategoryType = asyncHandler(async (req, res) => {
  ...
  if (!products.length) throw new ApiError(404, "No product found");

  res.status(200).json(new ApiResponse(200, products, `Products of '${selectedCategory}' fetched`));
});
...
const getProductById = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(new ApiResponse(200, product, `Product '${productId}' fetched`));
});
const getAllProducts = asyncHandler(async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1;
    ...
    if (req.query.categoryName) {
      query["category.name"] = { $regex: new RegExp(`^${req.query.categoryName}$`, "i") };
    }

    if (req.query.categoryType) {
      query["category.type"] = { $regex: new RegExp(`^${req.query.categoryType}$`, "i") };
    }
...
//   const query = {};
//   if (req.query.categoryName) query["category.name"] = req.query.categoryName;
//   if (req.query.categoryType) query["category.type"] = req.query.categoryType;
...
const searchProducts = asyncHandler(async (req, res) => {
  ...
  if (sort === 'asc' || sort === 'desc') {
    findQuery = findQuery.sort({ createdAt: sort === 'asc' ? 1 : -1 });
  }
...
const updateProduct = asyncHandler(async (req, res) => {
  ...
  if (!user || !user.isAdmin) throw new ApiError(403, "Unauthorized");
  ...
  if (req.file) {
    const fileUri = `data:${req.file.mimetype};base64,${Buffer.from(req.file.buffer).toString("base64")}`;
    const uploadResult = await uploadToCloudinary(fileUri, req.file.originalname);

    if (!uploadResult.success) throw new ApiError(500, "Image upload failed");
    updateFields.productImage = uploadResult.result?.secure_url;
  }

  if (!Object.keys(updateFields).length) throw new ApiError(400, "No fields to update");
...
const countProducts = asyncHandler(async (req, res) => {
  ...
  const menCount = await Product.countDocuments({ 'category.type': 'Men' });
  const womenCount = await Product.countDocuments({ 'category.type': 'Women' });
  const kidsCount = await Product.countDocuments({ 'category.type': 'Kids' });
  ...
  res.status(200).json({
    success: true,
    counts: {
      men: percent(menCount),
      women: percent(womenCount),
      kids: percent(kidsCount),
    },
  });
});
