...
router.post("/create-checkout-session", async (req, res) => {
  try {
    ...
    // ✅ Keep metadata short: Only store productId & quantity
...
// ✅ CREATE ORDER
const createOrder = async (customer, data) => {
  const items = JSON.parse(customer.metadata.cart); // Only has productId + quantity
...
router.post(
  ...
  async (req, res) => {
    ...
    // If using a Stripe webhook secret, add it to your .env
