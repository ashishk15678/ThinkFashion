...
//         const relatedResponse = await fetch(`http://localhost:8000/api/product/all?categoryName=${productData.data.category.name}&perPage=6`);
...
const Product = () => {
  ...
  const cartItems = useSelector((state) => state.cart.cartItems);
  const currentUser = useSelector((state) => state.user.currentUser); // ✅ Added
    ...
    const fetchProductAndRelated = async () => {
      try {
        ...
        const relatedResponse = await fetch(
          `http://localhost:8000/api/product/all?categoryName=${productData.data.category.name}&perPage=6`
        );
        if (!relatedResponse.ok) {
          throw new Error("Failed to fetch related products");
        }
    ...
    <div className="font-sans py-32 px-5 min-h-screen dark:bg-gray-900">
      <div className="p-6 lg:max-w-7xl max-w-2xl max-lg:mx-auto">
        <div className="grid items-start grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div>
            <h2 className="text-2xl font-extrabold text-gray-800 dark:text-gray-50">{product.name}</h2>
            <p className="text-gray-400 mt-2">Well-Versed Commerce</p>
            <p className="text-3xl font-bold text-gray-800 dark:text-gray-50 mt-4">${product.price}</p>
            ...
            {/* Sizes */}
            <h3 className="text-lg font-bold text-gray-800 dark:text-gray-50 mt-8">Choose a Size</h3>
            <div className="flex flex-wrap gap-4 mt-4">
            ...
            {/* Colors */}
            <h3 className="text-lg font-bold text-gray-800 dark:text-gray-50 mt-8">Choose a Color</h3>
            <div className="flex flex-wrap gap-4 mt-4">
              {product.color[0]?.match(/[a-zA-Z]+/g)?.map((color, index) => (
                <button
                  key={index}
        ...
        {/* Related products */}
        <div className="pt-10">
          <h2 className="text-xl font-bold dark:text-white mb-5">You May Also Like</h2>
